    ...
    def test_load_cumulative_stats_file_exists(self, temp_dir):
        ...
        with open(manager.cumulative_stats_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                "total_runs", "total_files_processed", "total_files_skipped",
                "total_files_errors", "total_original_size_bytes",
                "total_compressed_size_bytes", "total_space_saved_bytes", "last_updated"
            ])
            writer.writeheader()
            writer.writerow({
                "total_runs": "5",
